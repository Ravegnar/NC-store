...
export default function Weapon(props) {
  ...
  const pathname = `/NC-store/store/NSW/weapons/${details.id}`

  return (<>
    <div>
      <Link to={`/NC-store/store/NSW/weapons/${details.id}`} >
      ...
      <div className=" text-center">
        <div className="flex items-baseline relative justify-end text-center">
          <Link to={`/NC-store/store/NSW/weapons/${details.id}`} className="mt-4 text-2xl mx-auto hover:text-orange-500 text-center font-bold text-cyan-800">{details.name}</Link>
          {quantity > 0 && (
