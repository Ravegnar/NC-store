          ...
          <div className="flex flex-col lg:w-5/12 w-full max-w-[38rem] min-h-[30rem] lg:h-auto lg:px-6 p-6 lg:ml-4 h-auto bg-slate-800">
            {product.image && (<>
              ...
              <div className="flex w-full h-1/5">
                <img src={require(`${product.image}`)} url={product.image} className="w-1/2 h-full border-r-8 border-t-8 cursor-pointer border-slate-800 p-4 hover:p-1 object-contain object-center bg-slate-900 transform duration-300 ease-in-out" onClick={(e) => setSelectedImage(e.currentTarget.getAttribute('url'))} />
                <img src={require(`${product.image}`)} url={product.image} className="w-1/2 h-full border-l-8 border-t-8 cursor-pointer border-slate-800 p-4 hover:p-1 object-contain object-center bg-slate-900 transform duration-300 ease-in-out" onClick={(e) => setSelectedImage(e.currentTarget.getAttribute('url'))} />
              </div>
            </>)}
          </div>
        </div>
